...
function DocumentConversion(options, comments) {
    ...
    function converStyleForaAtributes(element, options) {
        ...
        if (element.alignment && element.alignment !== "both") {
            styleMap["text-align"] = element.alignment;
        }
        if (element.color) {
            styleMap["color"] = element.color;
        }
        if (element.bgColor) {
            styleMap["background-color"] = element.bgColor;
        }
        if (element.fontSize) {
            styleMap["font-size"] = element.fontSize + "px";
        }
        if (element.isUnderline) {
            styleMap["text-decoration"] = "underline";
        }
        var styleStr = Object.keys(styleMap)
            .map(function(item) {
                return item + ":" + styleMap[item];
            })
            .join(";");
        if (styleStr) {
